...
的曲线插点，且只放行这一次 0 → 1 固化更新。**旧库升级**（已有曲线、无 `sealed`
列、且带旧版“无条件拒绝 UPDATE”触发器）时，仓储严格按 **建表 → 丢弃全部旧触发器 →
加 `sealed` 列并回填 `1` → 建新触发器** 的顺序初始化：必须先清旧触发器，否则回填
`sealed=1` 的 UPDATE 会被旧触发器 ABORT 导致旧库打不开。升级后既有曲线一律为
`sealed=1`，同样不可追加；整个初始化对同一文件重复执行幂等。
